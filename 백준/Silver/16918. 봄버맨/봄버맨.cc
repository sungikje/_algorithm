...
/*
Q를 기반으로 시간을 관리하려고 했는데 아래 요소들을 고려하지 않아 실패했었음.
    - Q에 이미 들어가 있는 좌표가 주변 폭탄에 영향을 받아 터진 경우 새로 폭탄이 설치되면서 Q에 있는 시간 정보와 실제 추가된 시간 정보가 맞지 않아 오류 발생
        - time table을 별도로 정의하고 Q에서 꺼낼 때 비교하며 이를 보완
또한 초반에 Q를 1개로 두고 폭발을 관리 했었는데 이렇게 되면 각 폭탄들이 독립적으로 터지지 못해 Q를 2개로 늘렸다.
*/